...
const Bytes32InputItem = ({
    ...
    return (
        <>
            <Flex direction="row" alignSelf={"flex-start"} alignItems="baseline" w="100%">
            ...
            >
                <Input
                    ...
                    type="search"
                    value={
                        uiFragment.convertToBytes && uiFragment.value && isBytes(uiFragment.value)
                            ? stringToHex(uiFragment.value, {size: 32})
                            : uiFragment.value
                    }
                        ...
                        dispatchArguments({
                            value: uiFragment.convertToBytes
                                ? //  web3ctx.provider.utils.padLeft(
                                  stringToHex(event.target.value, {size: 32})
                                : // 32
                                  // )
                                  event.target.value,
                            index,
...
const NumberInputItem = ({
    ...
    return (
        <>
            {" "}
            <FormLabel mb="8px" wordBreak={"break-all"} w="fit-content" alignSelf={"flex-start"}>
                {uiFragment.label}
            </FormLabel>{" "}
    ...
    return (
        <>
            <Flex direction={"row"} alignItems="baseline" alignSelf={"flex-start"}>
                <FormLabel mb="8px" wordBreak={"break-all"} w="fit-content" alignSelf={"baseline"}>
                    {uiFragment.label}
                </FormLabel>{" "}
                <Select
                    size="sm"
...
const TupleInputItem = ({
    ...
    const dispatchTupleArguments = ({internalIndex, internalValue}: {internalIndex: any; internalValue: any}) => {
    ...
    return (
        ...
        >
            ...
            {"components" in abiItem &&
                abiItem.components?.map((internalProperty: AbiParameter, idx: number) => {
                    return (
                        <Web3MethodField
                            key={`tuple-${idx}`}
