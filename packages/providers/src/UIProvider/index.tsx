...
export const UIProvider = ({children, config}: {children: React.ReactNode; config: WebSiteConfig}) => {
    ...
    // *********** Sidebar states **********************
    const {collapseSidebar, toggleSidebar, collapsed, toggled, broken, rtl} = useProSidebar(); // Whether sidebar should be visible at all or hidden
    const [sidebarVisible, setSidebarVisible] = useStorage<boolean>(window.sessionStorage, "sidebarVisible", true);
    ...
    // Whether sidebar should be toggled in mobile view
    const [sidebarToggled, setSidebarToggled] = useStorage<boolean>(window.sessionStorage, "sidebarToggled", false);
