...
import {RequireWeb3} from "@peersky/react-components";
const AppLayout = (props: any) => {
    ...
    return (
        <Flex
            ...
            px="7%"
        >
            <Breadcrumb spacing="8px" pt={2} separator={<ChevronRightIcon color="grey.500" />}>
                {path.length !== 0 && (
                    <BreadcrumbItem>
                ...
                {path?.map((element, idx) => {
                    ...
                    return (
                        <BreadcrumbItem key={`bcl-${element}-${idx}`}>
                            <Link passHref={true} shallow href={{pathname: linkPath, query: {...query}}}>
                                <BreadcrumbLink
                                    as={"div"}
...
export const getLayout = (page: any) => getSiteLayout(<AppLayout>{page}</AppLayout>);
