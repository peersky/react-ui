...
import {AbiItemType, AbiFunction, AbiParameter} from "abitype";
import {AbiItem} from "viem";

const makeTupleField = (element: AbiParameter): UITupleFragmentField => {
    let tupleUI: UITupleFragmentField = {
        components: [],
        hide: false,
        label: element.name ?? "",
    };
    if (!("components" in element)) throw new Error("tuple has no components");
...
const isNumeric = (fragment: AbiParameter) => {
    return fragment.type.includes("int");
};
const makeUIFields = <T extends UIFragmentField>(
    ...
    if ("components" in element) {
        ...
        hide && element?.name && hide.includes(element?.name) && (retval.hide = true);
        ...
        const retval = {
            ...
            placeholder: (element?.name && argumentFields && argumentFields[element.name]?.placeholder) ?? element.name,
            hide: (!!element?.name && hide?.includes(element.name)) ?? false,
            value:
                (element.name && argumentFields && argumentFields[element.name]?.initialValue) ??
            ...
            label:
                (element.name && argumentFields && argumentFields[element.name]?.label) ??
...
const setArguments = (
    ...
    if (index !== undefined) {
        ...
        if (newState.inputs[index]?.type === "tuple" || newState.inputs[index]?.type === "tuple[]") {
            newState.ui[index] = value as UITupleFragmentField;
            return newState;
        } else {
            if (newState.inputs[index]?.type?.includes("int")) {
                let ui = newState.ui[index] as UINumberFragmentField | UINUmberFragmentFieldArray;
                ui.value = (value as string | []) ?? "";
                ui.valueIsEther = valueIsEther;
            } else {
                let ui = newState.ui[index] as UIStringFragmentField | UIStringFragmentFieldArray;
                ui.value = (value as string | []) ?? "";
...
export const useABIItemForm = (fragment: AbiFunction, argumentFields?: ArgumentFields, hideFields?: string[]) => {
    const initialState = React.useMemo(() => {
        const newState: UIFragment = {...fragment, ui: []};
        newState.inputs?.forEach((element: AbiParameter, index: number) => {
            if (newState.inputs && newState.inputs[index]) {
                newState.ui[index] = makeUIFields<UIFragmentField>(element, argumentFields, hideFields);
            }
        });
        return newState;
    ...
    const [state, dispatchArguments] = React.useReducer(setArguments, initialState);
        ...
        state?.inputs?.forEach((inputElement, index) => {
            returnedObject[index] =
                inputElement.type === "address"
                    ? viem.isAddress(viem.getAddress((state.ui[index].value as any as string) ?? "0x"))
                        ? viem.getAddress((state.ui[index] as any as UIStringFragmentField).value)
            ...
            if (inputElement.type && inputElement.type.includes("int")) {
                if (inputElement.type.includes("[]")) {
                    returnedObject[index] = returnedObject.map((value: string) =>
                        viem.parseUnits(value, (state.ui[index] as UINumberFragmentField).valueIsEther ? 18 : 1)
                    );
                } else {
                    returnedObject[index] = viem.parseUnits(
                        (state.ui[index] as UINumberFragmentField).value,
                        (state.ui[index] as UINumberFragmentField).valueIsEther ? 18 : 1
                    );
                }
            }
        });
